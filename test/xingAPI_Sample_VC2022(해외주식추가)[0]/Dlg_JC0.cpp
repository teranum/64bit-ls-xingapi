...
void CDlg_JC0::InitCtrls()
{
	...
	m_ctrlOutBlock.InsertItem( nRow++, "대비기호          " );
	...
	m_ctrlOutBlock.InsertItem( nRow++, "기초자산현재가    " );
}
...
LRESULT CDlg_JC0::OnXMReceiveRealData( WPARAM wParam, LPARAM lParam )
{
	...
	if( strcmp( pRealPacket->szTrCode, "JC0" ) == 0 )
	{
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sign       , sizeof( pOutBlock->sign        ), DATA_TYPE_STRING	   ) );    // 대비기호          
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->change     , sizeof( pOutBlock->change      ), DATA_TYPE_FLOAT , 2 ) );    // 전일대비          
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->drate      , sizeof( pOutBlock->drate       ), DATA_TYPE_LONG      ) );    // 등락율            
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->price      , sizeof( pOutBlock->price       ), DATA_TYPE_LONG      ) );    // 현재가            
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->open       , sizeof( pOutBlock->open        ), DATA_TYPE_LONG      ) );    // 시가              
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->high       , sizeof( pOutBlock->high        ), DATA_TYPE_LONG      ) );    // 고가              
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->low        , sizeof( pOutBlock->low         ), DATA_TYPE_STRING    ) );    // 저가              
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->cgubun     , sizeof( pOutBlock->cgubun      ), DATA_TYPE_LONG      ) );    // 체결구분          
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->mschecnt   , sizeof( pOutBlock->mschecnt    ), DATA_TYPE_FLOAT , 2 ) );    // 매수누적체결건수  
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->cpower     , sizeof( pOutBlock->cpower      ), DATA_TYPE_LONG      ) );    // 체결강도          
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->openyak    , sizeof( pOutBlock->openyak     ), DATA_TYPE_FLOAT , 2 ) );    // 미결제약정수량    
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->k200jisu   , sizeof( pOutBlock->k200jisu    ), DATA_TYPE_LONG      ) );    // KOSPI200지수      
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->theoryprice, sizeof( pOutBlock->theoryprice ), DATA_TYPE_FLOAT , 3 ) );    // 이론가            
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->kasis      , sizeof( pOutBlock->kasis       ), DATA_TYPE_LONG      ) );    // 괴리율            
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sbasis     , sizeof( pOutBlock->sbasis      ), DATA_TYPE_LONG      ) );    // 시장BASIS         
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->ibasis     , sizeof( pOutBlock->ibasis      ), DATA_TYPE_LONG      ) );    // 이론BASIS         
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->basprice   , sizeof( pOutBlock->basprice    ), DATA_TYPE_LONG      ) );    // 기초자산현재가
	}
