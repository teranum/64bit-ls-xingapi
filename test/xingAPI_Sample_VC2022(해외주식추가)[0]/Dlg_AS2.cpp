// Dlg_AS2.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CDlg_AS2, CDialog)

CDlg_AS2::CDlg_AS2(CWnd* pParent /*=NULL*/)
	: CDialog(CDlg_AS2::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlg_AS2)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}


void CDlg_AS2::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlg_AS2)
	DDX_Control(pDX, IDC_OUTBLOCK, m_ctrlOutBlock);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CDlg_AS2, CDialog)
	//{{AFX_MSG_MAP(CDlg_AS2)
...
/////////////////////////////////////////////////////////////////////////////
// CDlg_AS2 message handlers

BOOL CDlg_AS2::OnInitDialog() 
{
...
// 데이터를 받음
//--------------------------------------------------------------------------------------
LRESULT CDlg_AS2::OnXMReceiveRealData( WPARAM wParam, LPARAM lParam )
{
	LPRECV_REAL_PACKET pRealPacket = (LPRECV_REAL_PACKET)lParam;

	if( strcmp( pRealPacket->szTrCode, "AS2" ) == 0 )
	{
		LPAS2_OutBlock pOutBlock = (LPAS2_OutBlock)pRealPacket->pszData;
		int nRow = 0;
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sOrdNo             , sizeof( pOutBlock->sOrdNo             ), DATA_TYPE_LONG      ) );    // [long  ,   10] 주문번호          
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sOrdCndi           , sizeof( pOutBlock->sOrdCndi           ), DATA_TYPE_STRING    ) );    // [string,    1] 주문조건          
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sOrdprcPtnCode     , sizeof( pOutBlock->sOrdprcPtnCode     ), DATA_TYPE_STRING    ) );    // [string,    2] 호가유형코드       
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sShtnIsuNo         , sizeof( pOutBlock->sShtnIsuNo         ), DATA_TYPE_LONG      ) );    // [long  ,    9] 단축종목번호       
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sUnercQty          , sizeof( pOutBlock->sUnercQty          ), DATA_TYPE_STRING    ) );    // [string,   16] 미체결수량(주문)    
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sOrgOrdMdfyQty     , sizeof( pOutBlock->sOrgOrdMdfyQty     ), DATA_TYPE_LONG      ) );    // [long  ,   16] 원주문정정수량      
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sOrgOrdCancQty     , sizeof( pOutBlock->sOrgOrdCancQty     ), DATA_TYPE_LONG      ) );    // [long  ,   16] 원주문취소수량      
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sCommdaCode        , sizeof( pOutBlock->sCommdaCode        ), DATA_TYPE_STRING    ) );    // [string,    2] 통신매체코드       
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sOrdAcntNo         , sizeof( pOutBlock->sOrdAcntNo         ), DATA_TYPE_STRING    ) );    // [string,   20] 주문계좌번호       
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sCmsnAmtExecAmt    , sizeof( pOutBlock->sCmsnAmtExecAmt    ), DATA_TYPE_LONG      ) );    // [long  ,   16] 수수료체결금액      
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sCrdayRuseExecVal  , sizeof( pOutBlock->sCrdayRuseExecVal  ), DATA_TYPE_STRING    ) );    // [string,   16] 금일재사용체결금액   
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sOrdUserId         , sizeof( pOutBlock->sOrdUserId         ), DATA_TYPE_LONG      ) );    // [long  ,   16] 주문자ID         
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sExecTime          , sizeof( pOutBlock->sExecTime          ), DATA_TYPE_LONG      ) );    // [long  ,    9] 체결시각          
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sPchsAmt           , sizeof( pOutBlock->sPchsAmt           ), DATA_TYPE_STRING    ) );    // [string,   16] 매입금액          
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sSubstAmt          , sizeof( pOutBlock->sSubstAmt          ), DATA_TYPE_LONG      ) );    // [long  ,   16] 대용금           
		...
		m_ctrlOutBlock.SetItemText(  nRow++, 1, GetDispData( pOutBlock->sRuseAbleAmt       , sizeof( pOutBlock->sRuseAbleAmt       ), DATA_TYPE_STRING    ) );    // [string,   16] 재사용가능금액      
