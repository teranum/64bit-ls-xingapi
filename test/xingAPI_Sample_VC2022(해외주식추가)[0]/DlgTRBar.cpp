...
void CDlgTRBar::LoadTRList()
{
	...
	CString strRead;
	CString strTR;
	LPTR_WINDOW_TABLE pTable = GetTRWindowTableData();
	int nCount = GetTRWindowTableCount();
	for( int nIndex=0; nIndex<nCount; nIndex++ )
	{
		HTREEITEM hItem = NULL;
		for( int i=0; i<1; i++ )
		{
			if( strlen( pTable[nIndex].szMenu[i] ) == 0 )
			{
				continue;
			}

			HTREEITEM hFind = FindItem( hItem, pTable[nIndex].szMenu[i] );
			if( hFind == NULL )
			{
				hFind = m_ctrlTR.InsertItem( pTable[nIndex].szMenu[i], hItem ? hItem : TVI_ROOT );
			}
			hItem = hFind;
		}
		
		strTR.Format( "[%s] %s (%s/%s)", pTable[nIndex].szTrCode, pTable[nIndex].szDesc, pTable[nIndex].szMenu[1], pTable[nIndex].szMenu[2] );
