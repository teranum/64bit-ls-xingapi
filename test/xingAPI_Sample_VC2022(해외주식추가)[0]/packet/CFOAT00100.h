...
///////////////////////////////////////////////////////////////////////////////////////////////////
// 선물옵션 정상주문 ( SERVICE=CFOAT00100,ENCRYPT,SIGNATURE,HEADTYPE=B,CREATOR=원종혁,CREDATE=2022/02/16 14:20:59 )
#pragma pack( push, 1 )
...
typedef struct _CFOAT00100InBlock1
{
    ...
    char    FnoOrdPrc           [  27];    // [double, 27.8] 선물옵션주문가격                StartPos 43, Length 27
    char    OrdQty              [  16];    // [long  ,   16] 주문수량                        StartPos 70, Length 16
...
typedef struct _CFOAT00100OutBlock1
{
    ...
    char    FnoOrdPrc           [  27];    // [double, 27.8] 선물옵션주문가격                StartPos 54, Length 27
    char    OrdQty              [  16];    // [long  ,   16] 주문수량                        StartPos 81, Length 16
    char    CommdaCode          [   2];    // [string,    2] 통신매체코드                    StartPos 97, Length 2
    char    DscusBnsCmpltTime   [   9];    // [string,    9] 협의매매완료시각                StartPos 99, Length 9
    char    GrpId               [  20];    // [string,   20] 그룹ID                          StartPos 108, Length 20
    char    OrdSeqno            [  10];    // [long  ,   10] 주문일련번호                    StartPos 128, Length 10
    char    PtflNo              [  10];    // [long  ,   10] 포트폴리오번호                  StartPos 138, Length 10
    char    BskNo               [  10];    // [long  ,   10] 바스켓번호                      StartPos 148, Length 10
    char    TrchNo              [  10];    // [long  ,   10] 트렌치번호                      StartPos 158, Length 10
    char    ItemNo              [  16];    // [long  ,   16] 항목번호                        StartPos 168, Length 16
    char    OpDrtnNo            [  12];    // [string,   12] 운용지시번호                    StartPos 184, Length 12
    char    MgempNo             [   9];    // [string,    9] 관리사원번호                    StartPos 196, Length 9
    char    FundId              [  12];    // [string,   12] 펀드ID                          StartPos 205, Length 12
    char    FundOrdNo           [  10];    // [long  ,   10] 펀드주문번호                    StartPos 217, Length 10
