...
typedef struct _CFOAT00200InBlock1
{
    ...
    char    FnoOrdPrc           [  27];    // [double, 27.8] 선물옵션주문가격                StartPos 52, Length 27
    char    MdfyQty             [  16];    // [long  ,   16] 정정수량                        StartPos 79, Length 16
...
typedef struct _CFOAT00200OutBlock1
{
    ...
    char    FnoOrdPrc           [  27];    // [double, 27.8] 선물옵션주문가격                StartPos 61, Length 27
    char    MdfyQty             [  16];    // [long  ,   16] 정정수량                        StartPos 88, Length 16
    char    CommdaCode          [   2];    // [string,    2] 통신매체코드                    StartPos 104, Length 2
    char    DscusBnsCmpltTime   [   9];    // [string,    9] 협의매매완료시각                StartPos 106, Length 9
    char    GrpId               [  20];    // [string,   20] 그룹ID                          StartPos 115, Length 20
    char    OrdSeqno            [  10];    // [long  ,   10] 주문일련번호                    StartPos 135, Length 10
    char    PtflNo              [  10];    // [long  ,   10] 포트폴리오번호                  StartPos 145, Length 10
    char    BskNo               [  10];    // [long  ,   10] 바스켓번호                      StartPos 155, Length 10
    char    TrchNo              [  10];    // [long  ,   10] 트렌치번호                      StartPos 165, Length 10
    char    ItemNo              [  10];    // [long  ,   10] 아이템번호                      StartPos 175, Length 10
    char    MgempNo             [   9];    // [string,    9] 관리사원번호                    StartPos 185, Length 9
    char    FundId              [  12];    // [string,   12] 펀드ID                          StartPos 194, Length 12
    char    FundOrgOrdNo        [  10];    // [long  ,   10] 펀드원주문번호                  StartPos 206, Length 10
    char    FundOrdNo           [  10];    // [long  ,   10] 펀드주문번호                    StartPos 216, Length 10
